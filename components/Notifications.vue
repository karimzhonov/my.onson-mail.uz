...
export default {
    ...
    methods: {
        ...
        async select(item) {
            const { data } = await this.$api.patch(`/notification/${item.id}/read/`, {}, {}, false)
            item.read = true
            await this.$router.push(useLocalePath()(data.url))
        },
        async allCheck() {
            await this.$api.patch(`/notification/`, {}, {}, false)
            this.read = true
        }
    }
}
                ...
                <ScrollArea class="h-screen flex">
                    <div class="flex-1 flex flex-col gap-2 pt-0">
                        <TransitionGroup name="list" appear>
                            <button v-for="item of items.results" :key="item.id" class="flex flex-col items-start gap-2 rounded-lg border p-3 text-left text-sm transition-all hover:bg-accent" @click="select(item)">
                                <div class="flex w-full flex-col gap-1">
                                    <div class="flex items-center">
                                        ...
                                        <div class="flex items-center gap-2 ml-2">
                                            ...
                                            <span v-if="!item.read || read" class="flex h-2 w-2 rounded-full bg-blue-600" />
                                        </div>
